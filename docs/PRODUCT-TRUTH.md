...
| --- | --- | --- | --- |
| Supported OS versions | `project.yml` deployment targets | Requires macOS 15.4+ and iOS/iPadOS 26+; the Foundation Models intelligence tier (smarter titles, Smart Paste rewrites, Ask your clipboard) and Liquid Glass require macOS 26 and degrade to the documented deterministic fallbacks below it | `ReleaseMetadataTests`, `scripts/check-product-truth.sh`, `IntelligenceCapability` |
...
| Current direct download | GitHub release `v0.8.2` and its published DMG | v0.8.2 is Developer ID-signed, notarized, stapled, checksum-matched, and Gatekeeper-accepted; the 0.8 series is the first to embed a production CloudKit/Push provisioning profile, so the artifact is sync-capable; a fresh Free install still runs local-only because sync is Pro-gated and secure direct activation is not yet available | Version sync, mounted-DMG QA, profile validation, and explicit release-copy assertions |
...
| Direct-download sync entitlement | `SyncEnablement`, `LicenseActivationService`, `LicenseKeyPurchaseHandler`, public release configuration | Sync requires Pro. Activation, validation, revocation, and seat limits are delegated to Lemon Squeezy without embedding a payments secret. **On `main`, Pro is now attainable on the direct channel, so `shouldEnable` (`tier == .pro && iCloudAvailable && hasCloudKitEntitlement`) can turn on live CloudKit for a direct install — the published v0.8.2 artifact cannot, because it could not reach Pro. Tagging a release from this state ships reachable sync, so it MUST NOT be cut before the production CloudKit schema is deployed and the signed two-Mac matrix passes.** | Sync enablement truth table, activation-service tests, clean-install activation, and packaged-secret checks |
