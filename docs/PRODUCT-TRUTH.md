...
| Direct-download sync entitlement | `SyncEnablement`, `LicenseActivationService`, `LicenseKeyPurchaseHandler`, public release configuration | Sync requires Pro. Current source delegates activation, validation, revocation, and seat limits to Lemon Squeezy without embedding a payments secret; the published v0.8.2 artifact remains local-only for a fresh Free install until this path passes the signed release-candidate matrix and ships | Sync enablement truth table, activation-service tests, clean-install activation, and packaged-secret checks |
