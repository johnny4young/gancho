...
| --- | --- | --- | --- |
| Supported OS versions | `project.yml` deployment targets | Source requires macOS 15.4+ and iOS/iPadOS 26+; v0.8.3 is the first Sequoia-validated release, so the published direct download and the website both advertise that floor. Below macOS 26: model titles degrade to deterministic heuristics, model-backed Smart Paste rewrites/Translate and Ask are unavailable, deterministic PII redaction is independent and keeps working, and Liquid Glass falls back to opaque materials | `ReleaseMetadataTests`, `scripts/check-product-truth.sh`, `IntelligenceCapability` |
...
| Current direct download | GitHub release `v0.8.3` and its published DMG | v0.8.3 is Developer ID-signed, notarized, stapled, checksum-matched, and Gatekeeper-accepted; direct installs can buy, activate, revalidate, and deactivate Pro through Lemon Squeezy without an embedded payments secret, and eligible Pro installs can use encrypted iCloud sync | Version sync, mounted-DMG QA, profile validation, signed two-Mac matrix, and explicit release-copy assertions |
...
| Direct-download sync entitlement | `SyncEnablement`, `LicenseActivationService`, `LicenseKeyPurchaseHandler`, public release configuration | Sync requires Pro. Activation, validation, revocation, and seat limits are delegated to Lemon Squeezy without embedding a payments secret. In v0.8.3, `shouldEnable` (`tier == .pro && iCloudAvailable && hasCloudKitEntitlement`) can turn on live CloudKit for an activated direct install; the release is gated on the production schema and signed two-Mac matrix. | Sync enablement truth table, activation-service tests, clean-install activation, packaged-secret checks, and signed two-Mac matrix |
