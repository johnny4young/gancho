...
| Current direct download | GitHub release `v0.7.0` and its published DMG | v0.7.0 is Developer ID-signed, notarized, stapled, checksum-matched, and Gatekeeper-accepted; it preserves local encrypted history but does not embed a CloudKit provisioning profile, so that DMG is local-only | Version sync, mounted-DMG QA, and explicit release-copy assertions |
